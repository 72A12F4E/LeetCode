class Solution {
    var knownPaths: [String: [Coordinate]] = [:]
    
    func findWords(_ board: [[Character]], _ words: [String]) -> [String] {
        for word in words {
            if let path = exists(word[...], in: board) {
                knownPaths[word] = path
            }
        }
        return knownPaths.map(\.key)
    }
    ...
    struct Coordinate: Hashable {
        ...
        func adjacentCells(in bounds: Coordinate) -> Set<Coordinate> {
            Set([
    ...
    private func exists(
        ...
        // Short circuit memoization
        for (foundWord, path) in knownPaths {
            if word.starts(with: foundWord) {
                if let path = exists(
                    word.dropFirst(path.count),
                    in: board,
                    current: path.last!,
                    path: path,
                    bounds: .init(x: board.count, y: board.first!.count)
                ) {
                    return path
                }
            }
        ...
        for (x, row) in board.enumerated() {
            for (y, char) in row.enumerated() where char == word.first {
    ...
    private func exists(
        ...
        guard !word.isEmpty else { return path }
        
        let cells = current
            .adjacentCells(in: bounds)
            .subtracting(path)
            .filter { cell in
                board[cell.x][cell.y] == word.first
            }
        for cell in cells {
            if let path = exists(
                word.dropFirst(),
